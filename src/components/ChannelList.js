...
class   ChannelList extends Component           {
    ...
    switchChannel(param='next',id,channelid)    {
        ...
        if (param === 'next' && items[nextElem]){
            ...
            this.setState({itemChosen:nextElem,channelId: channelid, program: this.props.playList[nextElem].program,firstPressed:false});
            this.runningString('focus');
        }
        if (param === 'prev' && items[prevElem])
        {items[prevElem].focus();
            this.props.dispatch(setPrograms(parseProgram(this.props.playList[nextElem].program),this.props.playList[nextElem].program));
            this.setState({itemChosen:prevElem,channelId: channelid, program: this.props.playList[prevElem].program,firstPressed:false});
            this.runningString('focus');
    ...
    handleKey(e,elem,i)                         {
        switch (e.keyCode)                      {
            ...
            case 38:
            {
                ...
                break;
            case 13:                                {
                this.handleClick(this.props.playList[this.state.itemChosen],e,true);
                if (this.state.firstPressed)
                    setTimeout(this.setMenusVisibleFalse,300);
                this.setState({itemChosen:false,firstPressed:true});
            }
                break;
            case 32:
                this.handleClick(this.props.playList[this.state.itemChosen],e,true);
    ...
    componentDidUpdate(prevProps)               {
        ...
        //setChannel to first whan loaded
